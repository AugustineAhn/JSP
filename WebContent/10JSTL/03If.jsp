...
<body>
	<!-- 
		...
		-else문이 없기 때문에 조건이 여러개 일때는 표현이 어렵다.
		이대는 주로 choose문을 사용한다.
		형식]
			test: 조건
			var: test의 결과를 반환하는 변수설정
			scope: 영역설정(많이 사용하지 않음)
	 -->
	<h2>if태그</h2>
	...
	<!-- 
		JSTL의 if문은 else가 없기 때문에 참or거짓의 형태로 만들때는
		아래와 같이 참인 조건, 거짓인 조건으로 각각의 if문의 블럭을
		만들어야 한다.
	 -->
	<h3>문자열 비교</h3>
	...
	<!-- 
	-test 속성에 EL식이 아닌 일반값이 들어가면 무조건 false를 반환한다.
	...
	<!-- 로그인을 위한 파라미터가 있는 경우에만 검증을 위한 아래 문장을 실행한다. -->
