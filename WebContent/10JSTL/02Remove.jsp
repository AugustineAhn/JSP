...
<body>
	...
	영역에 저장된 속성을 제거하므로, 로그아웃에서 사용할 수도 있다.
	 -->
	<h2>remove 태그</h2>
	<c:set var="pageVar" value="페이지영역"></c:set>
	<c:set var= "pageVar" value = "리퀘스트영역1번 " scope="request"></c:set>
	<c:set var= "requestVar" value = "리퀘스트영역2번 " scope="request"></c:set>
	<c:set var= "sessionVar" value = "세션영역 " scope="session"></c:set>
	<c:set var= "appVar" value = "어플리케이션영역 " scope="application"></c:set>
	...
	영역 혹은 변수명이 설정이 잘못 되더라도 에러는 발생하지
	않는다.
	 -->
	<h3>remove로 삭제하기</h3>
	<c:remove var = "pageVar"></c:remove>
