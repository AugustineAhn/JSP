...
StringBuffer inter  =  new StringBuffer();//관심사항을 저장함
	...
	try{
		...
		*/
		...
		/* 
			파일업로드시 예외가 발생하면 request영역에 오류메시지를
 ...
 <body>
 	...
 	<ul>
 		...
 		<li>
 			파일크기:<%=(int)Math.ceil(mr.getFile("chumFile1").length() / 1024.0) %>kb</li>
 		<li>이미지표현:
 	...
 	<li>
 		...
 		<li>
 			파일크기:<%=(int)Math.ceil(mr.getFile("chumFile2").length() / 1024.0) %> kb</li>
 		<li>이미지표현:
