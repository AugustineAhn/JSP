...
%>
	...
	<!-- 
		input태그의 tabindex속성
		: 포커스 이동을 위해 탭(Tab)키를 눌렀을때 포커스가 이동하는 순서를
		지정한다. 지정되지 않으면 input태그가 작성된 순서대로
		이동하게된다.
	 -->
	<table border="1">
		<tr>
			<td>아이디</td>
			<td>
				<!-- 변수 user에 저장된 값이 있다면 vlaue속성에 삽입 -->
				<input type="text" name="user_id" tabindex="1"
					value="<%= user==null ? "" : user%>" />
				<!-- 변수 user에 저장된 값이 있을 때 checkbox의  checked속성을
				활성화한다. -->
				<input type="checkbox" name="id_save" value="Y"
						tabindex="3" <% if(user.length()!=0){ %> 
							checked="checked"
						<% } %>
						/>아이디 저장하기
		...
		<table border=1 width=400>
			<tr>
				<td align="center">
					...
					<%=session.getAttribute("USER_ID") %>님, 로그인하셧습니다.
					<br />
					즐거운 시간 보내세요^^*
					<br />
